...
-- Mapping GTA V vehicle colors to readable names
local vehicleColors = {
...
-- Mapping vehicle class IDs to user-friendly names
...
-- Categorize weapon class by hash
local function getWeaponClass(weaponHash)
    ...
    elseif group == 1548507267 then return "Heavy Weapon"
    else return "Unknown" end
end

-- Get vehicle details: plate, type, class, and color
local function getVehicleInfo(vehicle)
    if not vehicle or vehicle == 0 then return "Unknown", "Unknown", "Unknown", "Unknown" end
    ...
    local primaryColorIndex, _ = GetVehicleColours(vehicle)
    local primaryColor = vehicleColors[primaryColorIndex] or "Unknown"

    return plate, carType, primaryColor, vehicleClassNames[classId] or "Unknown"
end
                ...
                if not blacklistweapon then
                    ...
                    if not isSilenced then
                        ...
                        TriggerServerEvent(
                            ...
                            street1, street2, x, y, z,
                            weaponClass, isInVehicle,
